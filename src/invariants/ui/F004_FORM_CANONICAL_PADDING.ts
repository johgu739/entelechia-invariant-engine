...
export const F004_FORM_CANONICAL_PADDING: InvariantDefinition = {
  category: 'UI_FORM',
  code: 'F004',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    ...
    const paddingLeft = context.paddingLeft as number | undefined
    const paddingRight = context.paddingRight as number | undefined
    const paddingTop = context.paddingTop as number | undefined
    const paddingBottom = context.paddingBottom as number | undefined
    const expectedX = 24
    const expectedY = 16
    ...
    if (paddingLeft !== undefined && paddingRight !== undefined) {
      ...
      if (!paddingXCorrect) {
        throw new InvariantViolationError(
          ...
          'UI_FORM',
          'F004',
          `F004 violation: Form horizontal padding is ${paddingLeft}px/${paddingRight}px, expected ${expectedX}px. Form containers must use canonical padding.`,
          {
            paddingLeft,
            paddingRight,
            expectedX,
    ...
    if (paddingTop !== undefined && paddingBottom !== undefined) {
      ...
      if (!paddingYCorrect) {
        throw new InvariantViolationError(
          ...
          'UI_FORM',
          'F004',
          `F004 violation: Form vertical padding is ${paddingTop}px/${paddingBottom}px, expected ${expectedY}px. Form containers must use canonical padding.`,
          {
            paddingTop,
            paddingBottom,
            expectedY,
