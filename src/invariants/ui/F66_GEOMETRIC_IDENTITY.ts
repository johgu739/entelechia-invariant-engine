...
export const F66_GEOMETRIC_IDENTITY: InvariantDefinition = {
  category: 'UI_LAYOUT',
  code: 'F66',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    ...
    const actualGeometry = context.actualGeometry as Record<string, unknown> | undefined
    const verification = context.verification as { matches: boolean; differences?: unknown[] } | undefined
    ...
    if (actualGeometry && verification && !verification.matches) {
      throw new InvariantViolationError(
        ...
        'UI_LAYOUT',
        'F66',
        `F66 violation: Geometry signature mismatch for ${componentId}`,
        {
          componentId,
          realmId,
