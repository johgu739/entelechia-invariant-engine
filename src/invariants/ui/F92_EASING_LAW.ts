...
export const F92_EASING_LAW: InvariantDefinition = {
  category: 'UI_TYPOGRAPHY',
  code: 'F92',
  name: 'Easing Law',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const transitionEasings = context.transitionEasings as Array<{ element: string; easing: string; type: 'entry' | 'exit' | 'crossfade' }> | undefined
    ...
    if (transitionEasings && transitionEasings.length > 0) {
      ...
      if (violations.length > 0) {
        throw new InvariantViolationError(
          ...
          'UI_TYPOGRAPHY',
          'F92',
          'F92 violation: Invalid easing functions detected. Must use ease-out for entry, ease-in for exit, ease-in-out for crossfades.',
          {
            violations,
