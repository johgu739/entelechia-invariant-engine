...
export const F91_TEMPORAL_UNITY = {
    category: 'UI_TYPOGRAPHY',
    code: 'F91',
    name: 'Temporal Unity',
    ...
    enforce: (_node, context) => {
        const transitionDurations = context.transitionDurations;
        ...
        if (transitionDurations && transitionDurations.length > 0) {
            ...
            if (invalidDurations.length > 0) {
                throw new InvariantViolationError('UI_TYPOGRAPHY.F91', 'UI_TYPOGRAPHY', 'F91', `F91 violation: Invalid transition durations detected. Must use approved temporal grid: 120ms, 150ms, 180ms, 200ms. Found: ${invalidDurations.join(', ')}ms`, {
                    invalidDurations,
                    approvedDurations,
            ...
            if (maxDuration - minDuration > maxDurationDiff) {
                throw new InvariantViolationError('UI_TYPOGRAPHY.F91', 'UI_TYPOGRAPHY', 'F91', `F91 violation: Transition durations in same cluster differ by >30ms. Min: ${minDuration}ms, Max: ${maxDuration}ms, Diff: ${maxDuration - minDuration}ms`, {
                    transitionDurations,
                    minDuration,
                    maxDuration,
...
//# sourceMappingURL=F91_TEMPORAL_UNITY.js.map