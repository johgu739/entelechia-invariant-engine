...
export const F101_CANONICAL_TYPE_SCALE: InvariantDefinition = {
  category: 'UI_TYPOGRAPHY',
  code: 'F101',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const typeScaleViolations = context.typeScaleViolations as Array<{ element: string; semanticLevel: string; expected: string; actual: string }> | undefined
    ...
    if (typeScaleViolations && typeScaleViolations.length > 0) {
      throw new InvariantViolationError(
        ...
        'UI_TYPOGRAPHY',
        'F101',
        'F101 violation: Canonical type scale violations detected. All text must use approved semantic levels with correct Tailwind classes.',
        {
          typeScaleViolations,
