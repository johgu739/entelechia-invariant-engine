...
export const F005_FORM_SINGLE_SCROLL: InvariantDefinition = {
  category: 'UI_FORM',
  code: 'F005',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    ...
    if (scrollContainerCount !== undefined && scrollContainerCount !== expectedCount) {
      throw new InvariantViolationError(
        ...
        'UI_FORM',
        'F005',
        `F005 violation: Form has ${scrollContainerCount} scroll containers, expected ${expectedCount}. Forms must have exactly one scroll container.`,
        {
