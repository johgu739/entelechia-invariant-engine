...
export const A1_WORKSPACE_VIEW_ONLY_READS: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'A1_VIEW',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const hasDirectFetch = context.hasDirectFetch as boolean | undefined
    const hasApiCall = context.hasApiCall as boolean | undefined
    ...
    if (hasDirectFetch === true) {
      throw new InvariantViolationError(
        'DOMAIN_LOGIC.A1_VIEW',
        'DOMAIN_LOGIC',
        'A1_VIEW',
        'A1_WORKSPACE_VIEW_ONLY_READS violation: Direct fetch call detected in WorkspaceView. WorkspaceView must not make direct API calls. All data fetching must go through React Query hooks or intent layer.',
        {
          hasDirectFetch: true,
          sourceComponent,
    ...
    if (hasApiCall === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'A1_VIEW',
        'A1_WORKSPACE_VIEW_ONLY_READS violation: Direct API call detected in WorkspaceView. WorkspaceView must not make direct API calls. All data fetching must go through React Query hooks or intent layer.',
        {
          hasApiCall: true,
          sourceComponent,
          source: context.sourceFile || 'unknown',
