...
export const A7_AUTHCACHE_MIRRORS_SSOT: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'A7',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const cacheStale = context.cacheStale as boolean | undefined
    const backendSessionId = context.backendSessionId as string | undefined
    const cacheSessionId = context.cacheSessionId as string | undefined
    ...
    if (cacheStale === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'A7',
        'A7_AUTHCACHE_MIRRORS_SSOT violation: Auth cache is stale. Cache must be invalidated when backend session changes.',
        {
          cacheStale: true,
          backendSessionId,
          cacheSessionId,
    ...
    if (backendSessionId !== undefined && cacheSessionId !== undefined && backendSessionId !== cacheSessionId) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'A7',
        `A7_AUTHCACHE_MIRRORS_SSOT violation: Auth cache session ID (${cacheSessionId}) does not match backend session ID (${backendSessionId}). Cache must mirror backend state.`,
        {
          backendSessionId,
          cacheSessionId,
          mismatch: true,
