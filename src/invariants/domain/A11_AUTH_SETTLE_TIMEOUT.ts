...
export const A11_AUTH_SETTLE_TIMEOUT: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'A11',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const authSettled = context.authSettled as boolean | undefined
    const timeoutExceeded = context.timeoutExceeded as boolean | undefined
    const settleTime = context.settleTime as number | undefined
    const maxSettleTime = context.maxSettleTime as number | undefined
    ...
    if (timeoutExceeded === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'A11',
        `A11_AUTH_SETTLE_TIMEOUT violation: Auth state did not settle within timeout (${settleTime}ms / ${maxSettleTime}ms). Auth state must settle deterministically.`,
        {
    ...
    if (authSettled === false && (timeoutExceeded === false || timeoutExceeded === undefined) && settleTime !== undefined && maxSettleTime !== undefined) {
      if (settleTime > maxSettleTime) {
        throw new InvariantViolationError(
          ...
          'DOMAIN_LOGIC',
          'A11',
          `A11_AUTH_SETTLE_TIMEOUT violation: Auth state settle time (${settleTime}ms) exceeds maximum (${maxSettleTime}ms). Auth state must settle within timeout.`,
          {
            authSettled: false,
            settleTime,
            maxSettleTime,
