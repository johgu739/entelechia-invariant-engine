...
export const F28_NO_HARD_EXPIRY_WITHOUT_REFRESH = {
    category: 'DOMAIN_LOGIC',
    code: 'F28',
    ...
    enforce: (_node, context) => {
        const has401FromExpiredToken = context.has401FromExpiredToken;
        const hasSilentRefresh = context.hasSilentRefresh;
        const accessTokenExpired = context.accessTokenExpired;
        const refreshTokenValid = context.refreshTokenValid;
        ...
        if (has401FromExpiredToken === true && refreshTokenValid === true) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F28', 'DOMAIN_LOGIC', 'F28', 'F28_NO_HARD_EXPIRY_WITHOUT_REFRESH violation: Got 401 from expired access token when refresh token is valid. Access tokens must be refreshed silently BEFORE requests, never after 401.', {
        ...
        if (accessTokenExpired === true && hasSilentRefresh !== true && refreshTokenValid === true) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F28', 'DOMAIN_LOGIC', 'F28', 'F28_NO_HARD_EXPIRY_WITHOUT_REFRESH violation: Access token expired but silent refresh not attempted. Access tokens must be refreshed silently BEFORE requests when refresh token is valid.', {
...
//# sourceMappingURL=F28_NO_HARD_EXPIRY_WITHOUT_REFRESH.js.map