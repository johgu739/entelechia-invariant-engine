...
export const A2_AUTH_SPA_IDENTITY_PRESERVATION = {
    category: 'DOMAIN_LOGIC',
    code: 'A2',
    ...
    enforce: (_node, context) => {
        const hasHardReload = context.hasHardReload;
        const spaStatePreserved = context.spaStatePreserved;
        const sourceComponent = context.sourceComponent;
        // If hard reload detected, that's a violation
        if (hasHardReload === true) {
            throw new InvariantViolationError('DOMAIN_LOGIC.A2', 'DOMAIN_LOGIC', 'A2', 'A2_AUTH_SPA_IDENTITY_PRESERVATION violation: Hard reload (window.location.reload) detected during auth flow. Hard reloads destroy SPA state and WebSocket connections. SPA identity must be preserved.', {
                hasHardReload: true,
        ...
        if (spaStatePreserved === false) {
            throw new InvariantViolationError('DOMAIN_LOGIC.A2', 'DOMAIN_LOGIC', 'A2', 'A2_AUTH_SPA_IDENTITY_PRESERVATION violation: SPA state not preserved during auth flow. SPA identity (React state, WebSocket connections) must be preserved.', {
                spaStatePreserved: false,
                sourceComponent,
                source: context.sourceFile || 'unknown',
            });
...
//# sourceMappingURL=A2_AUTH_SPA_IDENTITY_PRESERVATION.js.map