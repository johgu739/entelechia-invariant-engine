...
export const F4_NODE_DEFINITIONS_ABSOLUTE = {
    category: 'DOMAIN_LOGIC',
    code: 'F4',
    ...
    enforce: (_node, context) => {
        const hasFallback = context.hasFallback;
        const hasHeuristic = context.hasHeuristic;
        const nodeId = context.nodeId;
        const sourceComponent = context.sourceComponent;
        // If fallback behavior detected, that's a violation
        if (hasFallback === true) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F4', 'DOMAIN_LOGIC', 'F4', `F4_NODE_DEFINITIONS_ABSOLUTE violation: Fallback behavior detected for node ${nodeId || 'unknown'}. Node definitions must be absolute - no fallbacks allowed.`, {
                nodeId,
                hasFallback: true,
                sourceComponent,
        ...
        if (hasHeuristic === true) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F4', 'DOMAIN_LOGIC', 'F4', `F4_NODE_DEFINITIONS_ABSOLUTE violation: Heuristic resolution detected for node ${nodeId || 'unknown'}. Node definitions must be absolute - no heuristics allowed.`, {
                nodeId,
                hasHeuristic: true,
                sourceComponent,
                source: context.sourceFile || 'unknown',
...
//# sourceMappingURL=F4_NODE_DEFINITIONS_ABSOLUTE.js.map