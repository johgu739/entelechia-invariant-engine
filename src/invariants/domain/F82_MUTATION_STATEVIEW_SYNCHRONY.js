...
export const F82_MUTATION_STATEVIEW_SYNCHRONY = {
    category: 'DOMAIN_LOGIC',
    code: 'F82',
    ...
    enforce: (_node, context) => {
        const lastMutationTimestamp = context.lastMutationTimestamp;
        const lastSSVRefreshTimestamp = context.lastSSVRefreshTimestamp;
        ...
        if (lastSSVRefreshTimestamp === undefined) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F82', 'DOMAIN_LOGIC', 'F82', 'F82_MUTATION_STATEVIEW_SYNCHRONY violation: Mutation occurred but StateView was not refreshed. StateView must be refreshed after every mutation.', {
        ...
        if (lastSSVRefreshTimestamp < lastMutationTimestamp) {
            ...
            throw new InvariantViolationError('DOMAIN_LOGIC.F82', 'DOMAIN_LOGIC', 'F82', `F82_MUTATION_STATEVIEW_SYNCHRONY violation: StateView refresh timestamp is before mutation timestamp. Refresh must happen after mutation. Drift: ${drift}ms`, {
                lastMutationTimestamp,
        ...
        if (refreshDelay > MAX_REFRESH_DELAY) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F82', 'DOMAIN_LOGIC', 'F82', `F82_MUTATION_STATEVIEW_SYNCHRONY violation: StateView refresh happened too long after mutation (${refreshDelay}ms > ${MAX_REFRESH_DELAY}ms). Refresh must happen immediately after mutation.`, {
                lastMutationTimestamp,
                lastSSVRefreshTimestamp,
                refreshDelay,
...
//# sourceMappingURL=F82_MUTATION_STATEVIEW_SYNCHRONY.js.map