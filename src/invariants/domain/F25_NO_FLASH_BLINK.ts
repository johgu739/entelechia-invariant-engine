...
export const F25_NO_FLASH_BLINK: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'F25',
  name: 'No Flash Blink',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const hasFlash = context.hasFlash as boolean | undefined
    const hasBlink = context.hasBlink as boolean | undefined
    const hasPlaceholderWithoutForm = context.hasPlaceholderWithoutForm as boolean | undefined
    const hasTransientDOMSwap = context.hasTransientDOMSwap as boolean | undefined
    const workspaceFrameRemounted = context.workspaceFrameRemounted as boolean | undefined
    const continuityBaselineInitialized = context.continuityBaselineInitialized as boolean | undefined
    ...
    if (hasFlash === true || hasBlink === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F25',
        'F25_NO_FLASH_BLINK violation: Flash or blink detected during transition. UI must maintain continuity during state transitions.',
        {
          hasFlash,
          hasBlink,
    ...
    if (hasPlaceholderWithoutForm === true && continuityBaselineInitialized === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F25',
        'F25_NO_FLASH_BLINK violation: Placeholder ViewModel replaced real ViewModel after continuity baseline was initialized. Placeholder ViewModels must not replace real ViewModels once continuity is established.',
        {
    ...
    if (hasTransientDOMSwap === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F25',
        'F25_NO_FLASH_BLINK violation: Transient DOM swap detected. DOM must maintain stable identity during transitions.',
        {
    ...
    if (workspaceFrameRemounted === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F25',
        'F25_NO_FLASH_BLINK violation: WorkspaceFrame remounted. WorkspaceFrame should only mount once per app lifecycle. Remounts cause flash/blink.',
        {
