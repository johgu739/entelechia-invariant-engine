...
export const F5_AUTH_SINGLE_SOURCE_OF_TRUTH = {
    category: 'DOMAIN_LOGIC',
    code: 'F5',
    ...
    enforce: (_node, context) => {
        const reactQueryAuth = context.reactQueryAuth;
        const zustandAuth = context.zustandAuth;
        const authCacheAuth = context.authCacheAuth;
        ...
        if (zustandAuth?.isAuthenticated !== undefined) {
            throw new InvariantViolationError('DOMAIN_LOGIC.F5', 'DOMAIN_LOGIC', 'F5', 'F5_AUTH_SINGLE_SOURCE_OF_TRUTH violation: Zustand stores auth status (isAuthenticated). Zustand must only store UI metadata (userEmail), never auth status. React Query is the single source of truth for auth status.', {
                reactQueryAuth,
                zustandAuth,
                authCacheAuth,
                sourceComponent,
        ...
        if (reactQueryAuth?.authenticated !== undefined && authCacheAuth?.authenticated !== undefined) {
            if (reactQueryAuth.authenticated !== authCacheAuth.authenticated) {
                throw new InvariantViolationError('DOMAIN_LOGIC.F5', 'DOMAIN_LOGIC', 'F5', 'F5_AUTH_SINGLE_SOURCE_OF_TRUTH violation: Auth cache diverges from React Query. Auth cache must mirror React Query (the single source of truth).', {
                    reactQueryAuth,
                    authCacheAuth,
                    sourceComponent,
                    source: context.sourceFile || 'unknown',
...
//# sourceMappingURL=F5_AUTH_SINGLE_SOURCE_OF_TRUTH.js.map