...
export const F27_SESSION_CONTINUITY: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'F27',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const has401Spam = context.has401Spam as boolean | undefined
    const requestCountAfterExpiry = context.requestCountAfterExpiry as number | undefined
    const hasGracefulHandling = context.hasGracefulHandling as boolean | undefined
    const sessionState = context.sessionState as string | undefined
    ...
    if (has401Spam === true) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F27',
        `F27_SESSION_CONTINUITY violation: 401 spam detected. ${requestCountAfterExpiry || 0} requests made after session expiry. Session expiration must be handled gracefully without repeated 401 errors.`,
        {
          has401Spam: true,
    ...
    if (requestCountAfterExpiry !== undefined && requestCountAfterExpiry > 1) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F27',
        `F27_SESSION_CONTINUITY violation: ${requestCountAfterExpiry} requests made after session expiry. Session expiration must be handled gracefully without repeated 401 errors.`,
        {
          has401Spam: true,
