...
export const F68_DASHBOARD_HEADER_UNIVERSALITY: InvariantDefinition = {
  category: 'DOMAIN_LOGIC',
  code: 'F68',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    ...
    const spacing = context.spacing as number | undefined
    const scrollBoundary = context.scrollBoundary as string | undefined
    ...
    if (violations.length > 0) {
      throw new InvariantViolationError(
        ...
        'DOMAIN_LOGIC',
        'F68',
        `F68_DASHBOARD_HEADER_UNIVERSALITY violation: Header does not match canonical dimensions. ${violations.join(', ')}`,
        {
          headerHeight,
          spacing,
          scrollBoundary,
