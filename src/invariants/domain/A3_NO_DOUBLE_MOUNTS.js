...
export const A3_NO_DOUBLE_MOUNTS = {
    category: 'DOMAIN_LOGIC',
    code: 'A3',
    ...
    enforce: (_node, context) => {
        const mountCount = context.mountCount;
        ...
        if (!isStrictModeDoubleMount && hasActualDoubleMount) {
            throw new InvariantViolationError('DOMAIN_LOGIC.A3', 'DOMAIN_LOGIC', 'A3', `A3_NO_DOUBLE_MOUNTS violation: Component mounted ${mountCount} times. Components should only mount once per render cycle (except React StrictMode double mount in dev). Double mounts cause duplicate effects, duplicate API calls, and state inconsistencies.`, {
                mountCount,
                isDev,
...
//# sourceMappingURL=A3_NO_DOUBLE_MOUNTS.js.map