...
export const F39_ROUTER_GUARD_SNAPSHOT_ALIGNMENT: InvariantDefinition = {
  category: 'SYSTEM_ROUTER',
  code: 'F39',
  ...
  enforce: (_node: HTMLElement | null, context: InvariantContext): void => {
    const guardUsesNetworkBeforeSnapshot = context.guardUsesNetworkBeforeSnapshot as boolean | undefined
    ...
    if (guardUsesNetworkBeforeSnapshot && hasPersistedSSV) {
      throw new InvariantViolationError(
        ...
        'SYSTEM_ROUTER',
        'F39',
        'F39 violation: Router guard uses network before persisted snapshot. Guard must prioritize persisted SystemStateView.',
        {
